...
private:
  ...
  void initialize_data(const Games& games) noexcept {
    ...
    for (const std::pair<std::size_t, PlayerName>& number_of_games_and_player_name : player_names_by_number_of_games_) {
      if (counter < Colors.size()) {
        data_.emplace_back(number_of_games_and_player_name.second, color(counter));
      } else {
        data_.emplace_back(number_of_games_and_player_name.second);
      }
    }
  ...
  std::unordered_map<PlayerName, EloRating> elo_ratings() const noexcept {
