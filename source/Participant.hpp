...
/// \brief Standing of a participant in a game. Includes a place, a player name,
/// a number of victory points, and a faction name.
class Participant {
public:
  Participant() noexcept {}

  Participant(const Place& place, const PlayerName& player_name,
  ...
  std::string print() const noexcept {
    return place_.print() + " " + player_name_.value() + " "
           + victory_points_.print() + " " + label(faction_name_);
  }

  bool operator==(const Participant& other) const noexcept {
    return place_ == other.place_ && player_name_ == other.player_name_
           && victory_points_ == other.victory_points_
           && faction_name_ == other.faction_name_;
  }
  ...
  struct sort {
    bool operator()(const Participant& participant_1,
                    const Participant& participant_2) const noexcept {
...
template<> struct hash<TI4Echelon::Participant> {
  size_t operator()(const TI4Echelon::Participant& participant) const {
    return hash<TI4Echelon::Place>()(participant.place())
           ...
           ^ hash<TI4Echelon::VictoryPoints>()(participant.victory_points())
           ^ hash<TI4Echelon::FactionName>()(participant.faction_name());
  }
};
